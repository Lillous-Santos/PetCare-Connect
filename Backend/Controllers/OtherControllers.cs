...
public class ConsultasController : ControllerBase
{
    ...
    public async Task<IActionResult> Criar([FromBody] CriarConsultaRequest req)
    {
        ...
        if (pet is null) return NotFound(new ApiResponse(false, "Pet não encontrado."));
...
public class VacinasController : ControllerBase
{
    ...
    public async Task<IActionResult> Criar([FromBody] CriarVacinaRequest req)
    {
        ...
        if (!string.IsNullOrEmpty(req.DataAplicacao))
            dataAplicacao = DateTime.ParseExact(req.DataAplicacao, "dd/MM/yyyy", null);
...
public class HistoricoController : ControllerBase
{
    ...
    public async Task<IActionResult> Adicionar([FromBody] CriarHistoricoRequest req)
    {
        ...
        if (!string.IsNullOrEmpty(req.DataEvento))
            dataEvento = DateTime.ParseExact(req.DataEvento, "dd/MM/yyyy", null);
...
{
    private readonly PetCareDb _db;
    public AvisosController(PetCareDb db) => _db = db;
    ...
    public async Task<IActionResult> Listar()
    {
        var lista = await _db.ListarAvisosAsync(UserId, Perfil, UserId);
        return Ok(new ApiResponse<IEnumerable<Aviso>>(true, "OK", lista));
    }

    [HttpPost]
    [Authorize(Roles = "Veterinario,Admin")]
    public async Task<IActionResult> Criar([FromBody] CriarAvisoRequest req)
    {
        var id = await _db.CriarAvisoAsync(new Aviso
        {
            Titulo  = req.Titulo,
            Texto   = req.Texto,
            Tipo    = req.Tipo,
        ...
        return Ok(new ApiResponse<int>(true, "Aviso criado.", id));
    }

    [HttpPost("{id}/lido")]
    public async Task<IActionResult> MarcarLido(int id)
    {
        await _db.MarcarAvisoComoLidoAsync(id, UserId);
        return Ok(new ApiResponse(true, "Marcado como lido."));
    }
}
